...
@section('title', 'Editar ParametroGeneral')
...
<div class="container-fluid px-4">
                        <h1 class="mt-4">Editar Parametro General</h1>
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item "><a href="{{route('panel')}}">Inicio</a> </li>
                            <li class="breadcrumb-item "><a href="{{route('genParametros.index')}}">Parametro General</a> </li>
                            <li class="breadcrumb-item active">Editar Parametro General</li>
                        </ol>

                        <div class="container w-100 border border-3 border-primary rounded p-4 mt-3">
<form action="{{route('genParametros.update',['genParametro'=>$genParametro])}}" method="post">
    @method('PATCH')
    @csrf
    <div class="row g-3">
        ...
        <div class="col-md-6 mb-2">
                    ...
                    @foreach ($estadoParametro as $item)
                        @if ($genParametro->state_parameters_id == $item->id)
                        <option selected value="{{$item->id}}" {{old('state_parameters_id') == $item->id ? 'selected' : ''}}>{{$item->name}}</option>
        ...
        <div class="col-md-6 mb-2">
                    ...
                    @foreach ($estadosCiviles as $item)
                        @if ($genParametro->civil_status_id == $item->id)
        ...
        <div class="col-md-6 mb-2">
                    ...
                    @foreach ($operadoraTelefonica as $item)
                        @if ($genParametro->phone_operator_id == $item->id)
                        <option selected value="{{$item->id}}" {{old('phone_operator_id') == $item->id ? 'selected' : ''}}>{{$item->name}}</option>
        ...
        <div class="col-md-6 mb-2">
                    ...
                    @foreach ($tipoContribuyente as $item)
                        @if ($genParametro->taxpayer_types_id == $item->id)
                        <option selected value="{{$item->id}}" {{old('taxpayer_types_id') == $item->id ? 'selected' : ''}}>{{$item->name}}</option>
        ...
        <div class="col-md-6 mb-2">
                    ...
                    @foreach ($tipoParametro as $item)
                        @if ($genParametro->type_parameters_id == $item->id)
                        <option selected value="{{$item->id}}" {{old('type_parameters_id') == $item->id ? 'selected' : ''}}>{{$item->name}}</option>
        ...
        <div class="col-md-6 mb-2">
                    ...
                    @foreach ($tipoSancion as $item)
                        @if ($genParametro->type_sanctions_id == $item->id)
                        <option selected value="{{$item->id}}" {{old('type_sanctions_id') == $item->id ? 'selected' : ''}}>{{$item->name}}</option>
